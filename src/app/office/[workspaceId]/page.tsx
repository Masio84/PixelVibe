...
export default function DynamicOfficePage() {
  ...
  const [workspaceName, setWorkspaceName] = useState<string>('Oficina');
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // Load recent messages for THIS workspace
      ...
      // Fetch online users for THIS workspace
      ...
      } else if (workspaceId === '00000000-0000-0000-0000-000000000000') {
        setWorkspaceName('Oficina Pública');
      }

      // Fetch active building layout for THIS workspace
      ...
      if (layoutData) {
        setMapData({
          ...
          spawn_x: layoutData.spawn_x ?? 8,
          spawn_y: layoutData.spawn_y ?? 8,
        });
        setLocalPosition({ x: layoutData.spawn_x ?? 8, y: layoutData.spawn_y ?? 8 });
      }
      setProfile(p);
    ...
    // Subscribe to avatar_positions for THIS workspace
    const channel = supabase
      .channel(`hud_positions_${workspaceId}`)
      .on('postgres_changes', {
        event: '*',
        schema: 'public',
